...
OvertakeSystem = ac.connect(OvertakeSystemharedData, true, ac.SharedNamespace.Shared)
OvertakeSystem_Fields = { "SystemAvailable", "status", "CoolDown", "CoolDownTime", "RemainingTime" }
...
RSSSupreme = ac.connect(RSSSupremeSystemsharedData, true, ac.SharedNamespace.Shared)
RSSSupreme_Fields = {
	"FuelMap", "ThrottleMap", "Throttleinput", "StartMode", "AttackWarning"
}

carScript = function(customData)
	customData.IgnitionMode = Ignition_RSS.Mode
	addAllData(OvertakeSystem, OvertakeSystem_Fields, 'OvertakeSystem_', customData)
	addAllData(RSSSupreme, RSSSupreme_Fields, 'RSSSupreme_', customData)
end
